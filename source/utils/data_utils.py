...
def clean_exposures(photos, c):
    ...
    for i in idx:
        a = photos['exif_exposure_time'].iloc[i].split(
            c)[0].replace("Second", "").replace(" ", "")
...
def clean_focal(photos, c):
    ...
    for i in idx:
        ...
        a = photos['exif_focal_length'].iloc[i].replace(
            " ", "").replace(delimiters[0], ".0")
...
def clean_apertures(photos, c):
    ...
    for i in idx:
        ...
        if 'inf' in a:
            ...
            photos['exif_aperture_value'].iloc[i] = a.replace(
                delimiters[0], ".").replace(delimiters[1], "").replace(delimiters[5], "")
...
def train_valid_test_split(dataset):
    '''
    Split training, validation, test sets 90/5/5
    :param: dataset, in pandas format
    '''

    train_valid_test_dataset = dataset.dropna(axis=0, inplace=False, subset=[
                                                                    'exif_iso', 'iso_noise_bin_label', 'exif_focal_length', 'exif_exposure_time', 'exif_aperture_value', 'orientation', 'iso_noise_label', 'DoF', 'DoF_bin', 'exposure_label', 'focal_label', 'focal_label_bin'])


    # test with NaN = full - not null indices
    test_nan = dataset.drop(
    index=train_valid_test_dataset.index)

    # Train/valid/test -> 90/5/5
    holdout_length = int(train_valid_test_dataset.shape[0] * 10/100)
    limit = train_valid_test_dataset.shape[0] - holdout_length
    valid_test_len = int(
        (train_valid_test_dataset.shape[0] - limit)/2)
...
def main():
    ...
    process_new_dataset(
        '../../dataset/unsplash-dataset-v1.1.0/',
        '/media/steve/Data2/unsplash-dataset/unsplashed-full/', 
        '../../dataset/new_photos.csv'
    )
